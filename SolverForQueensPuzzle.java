    ...
    private void recordSolutionsStarted() {
		...
		if (inProgress.accept()) {
			...
			solutions.add(snapshot);
            		//System.out.println( "  for debugging: base case detected for solution"
			...
			//System.out.println( "  for debugging: base case detected for no solution"
                            //  + System.lineSeparator()
                              //+ inProgress
                              //);
		}
			...
			for (int col = 0; col < inProgress.ranks(); col++) {
				inProgress.populate(col);
				nBoardsConsidered++;
				recordSolutionsStarted();
				inProgress.depopulate();
			}
			
            //System.out.println( "  for debugging: recursive case detected for legal but incomplete"
              //                + System.lineSeparator()
                //              + inProgress
                  //            );
		}
